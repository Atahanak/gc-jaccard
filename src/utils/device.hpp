  ...
  if (hwDevices.size() > 0) {
    for (auto &d : hwDevices) {
      if (d.attach()) {
        ...
        //std::string error = "IPU id " + d.getId() + " failed to attach.";
        throw std::runtime_error("IPU id ? failed to attach.");
      }
    }
  }
...
inline
poplar::Device getDeviceFromOptions(int num_ipus, const bool use_IPU_model) {
