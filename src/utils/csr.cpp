...
unsigned long randn(void* s) { //xorshf96          //period 2^96-1
  unsigned long t;
  seed* myxyz= (seed*)s;
  myxyz->x ^= myxyz->x << 16;
  myxyz->x ^= myxyz->x >> 5;
...
unsigned int sample_node_alias(VID * alias_v, char * alias_p, unsigned int size, unsigned int r1, unsigned long r2){
...
unsigned int get_neighbor(VID * d_V, E_T * d_A, VID v, unsigned long rn){
